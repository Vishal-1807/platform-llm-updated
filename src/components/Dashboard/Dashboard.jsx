...
export const Dashboard = () => {
  const GrafanaApiUrl = "https://api.workbenchai.aubrant.com/cibi";
  ...
  const handleChange = (e) => {
    ...
    if (ProjectFilter === "llm") {
      ...
      const single = `https://api.workbenchai.aubrant.com/cibi/grafana/d/a2159148-e5c7-4204-a1e6-fa614b5ebe6b/kubecost-llm?orgId=1&refresh=10s&var-namespace=${selectedValue}-train&var-namespace=${selectedValue}-infer&var-namespace=${selectedValue}-preprocess&from=now-20d&to=now&theme=light&kiosk&width=450`;
      setFinalURL(single);
    } else {
  ...
  const GetFilteredExperiments = async (e) => {
    try {
      ...
      const namespaces = filteredExperiments
        .map((exp) => generateNamespaces(exp))
        .join("");
      const finalUrl = `https://api.workbenchai.aubrant.com/cibi/grafana/goto/y9dWnjgDg?orgId=1&from=now-20d&to=now&theme=light&kiosk&width=450`;
      setFinalURL(finalUrl);
    } catch (error) {
      console.error("Error fetching experiments:", error);
    }
